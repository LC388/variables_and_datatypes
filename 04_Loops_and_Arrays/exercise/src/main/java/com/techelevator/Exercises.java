...
public class Exercises {
	...
	public boolean sameFirstLast(int[] nums) {
		int firstEqual = nums[0];
		int lastEqual = nums[nums.length-1];
	...
	public boolean commonEnd(int[] a, int[] b) {
		int firstAEqual = a[0];
		int lastAEqual = a[a.length -1];
		int firstBEqual = b[0];
		int lastBEqual = b[b.length - 1];
		if (firstAEqual == firstBEqual || lastAEqual == lastBEqual){
			return true;
		}
		return false;
	}
